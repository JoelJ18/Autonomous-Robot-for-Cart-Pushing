...
class DStar:
    ...
    def euc_dist (self):
        ...
        p2 = np.array(list(((63-self.path[0].row), self.path[0].col)))
    ...
    def run(self):
        ...
        while (cur_node.parent!=None):
            ...
            print ('Odometry')
            self.msg = rospy.wait_for_message('/base_pose_ground_truth', Odometry, timeout=None)
            ...
            while self.euc>1.5: 
                ...
                self.curpos = [self.msg.pose.pose.position.x, self.msg.pose.pose.position.y]
                pos = state()
                pos.x = 63-self.path[0].row
                pos.y = self.path[0].col
